...
def provide_ui() -> None:
    ...
    if len(transactions) == 0:
        mb.showerror(
            window_title,
            "Нечего фильтровать, либо при чтении произошла ошибка"
        )
    ...
    if filter_match and not filter_match.isspace():
        ...
        mb.showinfo(
            window_title,
            f"Отфильтровано по статусу {filter_match}"
        )
    ...
    if do_date_sort:
        ...
        if reverse:
            mb.showinfo(
                window_title,
                "Выполнена фильтрация в порядке,\n"
                "обратном историческому (по убыванию даты)"
            )
        else:
            mb.showinfo(
                window_title,
                "Выполнена фильтрация в\n"
    ...
    if filter_match and not filter_match.isspace():
        ...
        mb.showinfo(
            window_title,
            f"Выполнена фильтрация по вхождению\n"
            f"'{filter_match}' в описание")

    for t in transactions:
        try:
            date = datetime.datetime.strptime(
                extract(t, ("date",)), '%Y-%m-%dT%H:%M:%SZ'
            ).strftime('%d.%m.%Y')
        except ValueError: # operations.json format
            date = datetime.datetime.strptime(
                extract(t, ("date",)), '%Y-%m-%dT%H:%M:%S.%f'
            ).strftime('%d.%m.%Y')
        except TypeError:
        ...
        if not currency_code:
            currency_code = extract(t, ("operationAmount", "currency", "code",))
        amount = extract(t, ("amount",))
        if not amount:
            amount = extract(t, ("operationAmount", "amount",))
